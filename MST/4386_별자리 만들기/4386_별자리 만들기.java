...
public class Main {
  ...
  public static void main(String[] args) throws IOException{
    ...
    for (int i = 0; i < N; i++){
      ...
      xloc[i] = Double.parseDouble(st.nextToken());
      yloc[i] = Double.parseDouble(st.nextToken());
    }

    // make adjList
    for (int i = 0; i < N-1; i++){
      for (int j = i+1; j < N; j++){
        double distance = Math.sqrt(Math.pow(xloc[i]-xloc[j], 2) + Math.pow(yloc[i]-yloc[j], 2));
    ...
    // calculate answer
    for (Edge e: kmst.tree) answer += e.weight;
